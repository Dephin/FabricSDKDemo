    ...
    async recordCredit (ctx) {
		...
		let result = await Fabric_Client.newDefaultKeyValueStore({
			...
			const request = {
				...
				chaincodeId: 'schoolroll',
				fcn: 'recordCredit',
				args: [reqBody.userID, reqBody.username, reqBody.term, reqBody.lesson, reqBody.credit],
				chainId: 'mychannel',
				txId: tx_id
			};
			...
			if (isProposalGood) {
				...
				// is required bacause the event registration must be signead
